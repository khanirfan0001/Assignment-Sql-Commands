...
SELECT first_name,last_name,sum(amount)Total_Amount,RANK() OVER(ORDER BY amount )Rank_ 
FROM payment INNER JOIN rental r ON payment.rental_id = r.rental_id 
INNER JOIN customer c ON c.customer_id = r.customer_id GROUP BY amount ;

--  2. **Calculate the cumulative revenue generated by each film over time.**

SELECT f.title,p.payment_date,SUM(p.amount) OVER(PARTITION BY f.film_id 
ORDER BY p.payment_date) cumulative_revenue
...
SELECT customer_id, SUM(amount) total_spending FROM (SELECT customer_id, amount,
PERCENT_RANK() OVER (ORDER BY SUM(amount) DESC) AS percentile_rank FROM payment
GROUP BY customer_id) AS customer_spending WHERE percentile_rank <= 0.2
